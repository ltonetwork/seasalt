...
import com.ltonetwork.seasalt.Utils;
...
//    public KeyPair keyPair() throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
//        SecureRandom srSeed = new SecureRandom();
//
//        KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("ECDSA", "BC");
//        ECGenParameterSpec ecGenParameterSpec = new ECGenParameterSpec("secp256k1");
//
//        keyPairGenerator.initialize(ecGenParameterSpec, srSeed);
//
//        java.security.KeyPair test = keyPairGenerator.generateKeyPair();
//
//        return new KeyPair(test.getPublic().getEncoded(), test.getPrivate().getEncoded());
//    }
    ...
    public ECDSASignature signDetached(byte[] msg, byte[] privateKey) {
        ...
        byte[] rArr = Utils.integerToOctetString(signature[0], 32);
        byte[] sArr = Utils.integerToOctetString(toCanonicalised(signature[1]), 32);
    ...
    public boolean verify(byte[] msg, ECDSASignature signature, byte[] publicKey) {
        if (signature.getBytes().length == 65) {
            return verifyRecoveryKey(msg, signature, publicKey);
        } else if (signature.getBytes().length == 64) {
            return verifyNoRecoveryKey(msg, signature, publicKey);
        } else {
    ...
    public boolean verify(byte[] msg, byte[] signature, byte[] publicKey) {
        ...
        if (signature.length == 65) {
            byte[] v = new byte[1];
            System.arraycopy(signature, s.length, v, 0, v.length);
            return verify(msg, new ECDSASignature(r, s, v), publicKey);
    ...
    private boolean verifyRecoveryKey(byte[] msg, ECDSASignature signature, byte[] publicKey) {
        BigInteger pubKeyRecovered = signedMessageToKey(msg, signature);

        return Arrays.equals(publicKey, pubKeyRecovered.toByteArray());
    }

    private boolean verifyNoRecoveryKey(byte[] msg, ECDSASignature signature, byte[] publicKey) {
        ECDSASigner signer = new ECDSASigner();
    ...
    /**
     * Credits to the web3j project.
     * https://github.com/web3j/web3j/blob/master/crypto/src/main/java/org/web3j/crypto/Sign.java#L4
     */
     ...
     */
    public BigInteger recoverFromSignature(int recId, BigInteger r, BigInteger s, byte[] message) {
        assert (recId >= 0);
        assert (r.signum() >= 0);
        assert (s.signum() >= 0);
        assert (message != null);
        ...
        BigInteger prime = findCurvePrime(curveString);
    ...
    private BigInteger findCurvePrime(String curve) {
        BigInteger ret;
        switch (curve) {
            case "secp128r1":
                ret = SecP128R1Curve.q;
                break;
            case "secp256k1":
                ret = SecP256K1Curve.q;
                break;
            case "secp160k1":
                ret = SecP160K1Curve.q;
                break;
            case "secp160r1":
                ret = SecP160R1Curve.q;
                break;
            case "secp160r2":
                ret = SecP160R2Curve.q;
                break;
            case "secp192k1":
                ret = SecP192K1Curve.q;
                break;
            case "secp192r1":
                ret = SecP192R1Curve.q;
                break;
            case "secp224k1":
                ret = SecP224K1Curve.q;
                break;
            case "secp224r1":
                ret = SecP224R1Curve.q;
                break;
            case "secp256r1":
                ret = SecP256R1Curve.q;
                break;
            case "secp384r1":
                ret = SecP384R1Curve.q;
                break;
            case "secp521r1":
                ret = SecP521R1Curve.q;
                break;
            default:
     ...
     */
    private BigInteger signedMessageToKey(byte[] message, ECDSASignature signatureData) {
        ...
        int recId = header - 27;
        BigInteger key = recoverFromSignature(recId, new BigInteger(1, r), new BigInteger(1, s), message);
        if (key == null) {
     ...
     * BIP62</a>.
     */
    public boolean isCanonical(BigInteger s) {
